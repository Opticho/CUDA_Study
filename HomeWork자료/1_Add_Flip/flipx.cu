...
cudaError_t flip8WithCuda(unsigned char* input, unsigned char* output, int xSize, int ySize);
...
void flip8WithC(unsigned char* in, unsigned char* out, int xSize, int ySize) {
	size_t x, y;
	for (y = 0; y < ySize; y += 1) {
		for (x = 0; x < xSize; x += 1) {
			size_t in_pos = y * xSize + x;
			size_t out_pos = y * xSize + xSize - x - 1;
...
__global__ void kernelFlipX8(unsigned char* input, unsigned char* output, int size) {
	...
	if (index < (size)) {
		output[yLoc * xWidth + xWidth - xLoc - 1] = input[index];
	}
}
...
int main() {
	...
	flip8WithC(input, GoldOutput, xSize, ySize);
	// Add vectors in parallel.
	cudaError_t cudaStatus = flip8WithCuda(input, CudaOutput, xSize, ySize);
...
cudaError_t flip8WithCuda(unsigned char* input, unsigned char* output, int xSize, int ySize) {
	...
	kernelFlipX8 __dim__(blocks, threads) (dev_input, dev_output, xysize);
