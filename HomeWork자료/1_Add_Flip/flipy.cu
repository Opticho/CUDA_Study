...
void flip8WithC(unsigned char* in, unsigned char* out, int xSize, int ySize) {
	...
	for (y = 0; y < ySize; y += 1) {
		for (x = 0; x < xSize; x += 1) {
			...
			size_t out_pos = (ySize - y - 1) * xSize + x;
...
__global__ void kernelFlipY8(unsigned char* input, unsigned char* output, int size) {
	...
	if (index < (size)) {
		output[(yWidth - yLoc - 1) * xWidth + xLoc] = input[index];
	}
}
...
cudaError_t flip8WithCuda(unsigned char* input, unsigned char* output, int xSize, int ySize) {
	...
	kernelFlipY8 __dim__(blocks, threads) (dev_input, dev_output, xysize);
