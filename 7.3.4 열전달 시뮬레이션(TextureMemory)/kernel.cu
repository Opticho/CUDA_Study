...
int main(void) {
	...
	cudaBindTexture(&offset, refConstSrc, data.dev_constSrc, &desc, imageSize);
	cudaBindTexture(&offset, refIn, data.dev_inSrc, &desc, imageSize);
	cudaBindTexture(&offset, refOut, data.dev_outSrc, &desc, imageSize);
